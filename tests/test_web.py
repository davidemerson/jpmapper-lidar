...
from jpmapper.web.models import AnalyzeRequest, PointCoord
...
class TestAnalyzeEndpoint:
    def test_analyze_success(self, client):
        ...
        with patch("jpmapper.web.routes.analyze_los", return_value=mock_result), \
    ...
    def test_analyze_outside_dsm(self, client):
        ...
        with patch("jpmapper.web.routes.analyze_los", side_effect=NoDataError("Outside DSM")):
            resp = client.post("/api/analyze", json={
                "point_a": {"lat": 40.689, "lon": -73.986},
                "point_b": {"lat": 40.694, "lon": -73.990},
            })
